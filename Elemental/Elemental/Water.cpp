...
Water::Water(string c, int h, int da, int off, int def) {
	color = c;
	HP = h;
	DAM = da;
	offense = off;
...
Node* Water::changeElement(string color) {
	if(color.compare("f") == 0) {
		...
		cout << "You were already WATER... wated turn!" << endl;
		cout << "" << endl;
		return this;
	}
